...
- 前馈神经网络（Feedforward Neural Network，FNN），即数据从输入层单向流动到输出层，无循环结构，各层之间通过全连接或特定方式传递信息。其中多层感知机（Multi-Layer Perceptron，MLP）是最常见的形式：在典型的全连接结构中，当前层的每个神经元接收上一层所有神经元的输出，并将计算结果传递给下一层，如图2.1所示：
...
前馈神经网络（Feed Forward Neural Network，下简称 FNN），也就是我们在上一节介绍的数据从输入层单向流向输出层、相邻层之间进行信息传递的网络结构。在典型的全连接 FNN 中，当前层的每个神经元接收上一层所有神经元的输出，并将计算结果传递给下一层。每一个 Encoder Layer 都包含一个上文讲的注意力机制和一个前馈神经网络。前馈神经网络的实现是较为简单的：
